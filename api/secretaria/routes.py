...
from api.shared.rag import get_secretaria_context
from api.secretaria.prompt import build_secretaria_system_prompt
from api.shared.db import criar_ou_carregar_sessao, persistir_mensagem

secretaria_bp = Blueprint('secretaria_bp', __name__)

@secretaria_bp.route('/extract-name', methods=['POST'])
def extract_name():
    try:
        dados = request.json
...
@secretaria_bp.route('/chat', methods=['POST'])
def chat_secretaria():
    try:
        dados = request.json
        mensagem_aluno = dados.get("mensagem")
        ...
        # Chama a função de RAG com a pergunta do usuário (se aplicável)
        conteudo_documentos_rag = get_secretaria_context(mensagem_aluno)
        ...
        try:
            ...
            if backend == "supabase":
                url = f"{os.environ.get('SUPABASE_URL', '').rstrip('/')}/rest/v1/configuracoes_agentes?select=regras_customizadas&id=eq.secretaria"
        ...
        system_prompt = build_secretaria_system_prompt(nome, "", "", "", "", conteudo_documentos_rag, regras_customizadas)

        # 1. Tenta criar ou carregar a sessão no Supabase para guardar o perfil
        metadados = {}
        
        try:
            sessao_db = criar_ou_carregar_sessao(sessao_id, "secretaria", nome, metadados)
            if sessao_db:
