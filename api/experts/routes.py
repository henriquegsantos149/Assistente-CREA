...
experts_bp = Blueprint('experts_bp', __name__)

@experts_bp.route('/extract-name', methods=['POST'])
def extract_name():
    try:
        dados = request.json
...
@experts_bp.route('/chat', methods=['POST'])
def chat_experts():
    try:
        dados = request.json
        mensagem_aluno = dados.get("mensagem")
        ...
        try:
            ...
            if backend == "supabase":
                url = f"{os.environ.get('SUPABASE_URL', '').rstrip('/')}/rest/v1/configuracoes_agentes?select=regras_customizadas&id=eq.cursos"
        ...
        system_prompt = build_experts_system_prompt(nome, "", "", "", "", conteudo_documentos_rag, regras_customizadas)

        # 1. Tenta criar ou carregar a sessão no Supabase para guardar o perfil
        metadados = {}
        
        try:
            sessao_db = criar_ou_carregar_sessao(sessao_id, "cursos", nome, metadados)
            if sessao_db:
