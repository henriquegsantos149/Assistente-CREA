document.addEventListener('DOMContentLoaded', () => {
    ...
    function startOnboarding() {
        ...
        const welcomeParts = [
            `Olá! Que bom te ver por aqui. Eu sou o seu GeoExpert da Ambiental Pro, e estou pronto para ser seu parceiro de estudos! Você pode me pedir orientações sobre QGIS, debater sobre mapas ou tirar dúvidas técnicas avançadas em geotecnologias.`,
            `Para começar nossa parceria, como você se chama?`
        ];
    ...
    async function extractNameViaLLM(userMessage) {
        const response = await fetch('/api/experts/extract-name', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
