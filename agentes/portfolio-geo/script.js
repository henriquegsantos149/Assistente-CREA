document.addEventListener('DOMContentLoaded', () => {
    ...
    function startOnboarding() {
        ...
        const welcomeParts = [
            `Olá! Seja muito bem vindo(a). Eu sou o seu Assistente de Portfólio, você pode me enviar os seus projetos para que eu avalie; ou me pedir um plano para construir o seu portfólio, e vou te acompanhar passo a passo.`,
    ...
    chatForm.addEventListener('submit', async (e) => {
        ...
        processUserInput(text, text, file);
    });
    ...
    async function processUserInput(value, displayLabel, file = null) {
        const session = currentChatSession;

        let userMsg = displayLabel;
        if (file) {
            userMsg = userMsg ? `${userMsg} (Anexo: ${file.name})` : `(Anexo: ${file.name})`;
        }
        addMessage('user', userMsg || "Enviou um arquivo");
        hideInput();
        ...
        if (onboardingState === 'NAME') {
            ...
            await displaySequentialMessages(
                [`Tudo certo, **${userData.nome}**! Como posso te ajudar com o seu portfólio hoje?`],
                session
            );
            if (currentChatSession === session) {
                showTextInput('Digite sua dúvida ou envie um arquivo...');
        ...
        if (onboardingState === 'READY') {
            ...
            try {
                const response = await processLLMMessage(value, file);
                if (currentChatSession !== session) return;
                removeMessage(typingId);
                setLoading(false);
    ...
    async function processLLMMessage(userMessage, file) {
        ...
        if (file) {
            ...
            formData.append('formacao', userData.formacao);
            formData.append('ano', userData.ano);
            formData.append('hasCrea', userData.hasCrea);
            if (sessaoId) formData.append('sessao_id', sessaoId);
            formData.append('historico', JSON.stringify(historicoChat));
        ...
        const response = await fetch('/api/portfolio_geo/chat', fetchOptions);
    ...
    async function extractNameViaLLM(userMessage) {
        const response = await fetch('/api/portfolio_geo/extract-name', {
            method: 'POST',
