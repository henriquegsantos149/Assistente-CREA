document.addEventListener('DOMContentLoaded', () => {
    ...
    async function processUserInput(value, displayLabel) {
        ...
        if (onboardingState === 'NAME') {
            ...
            await displaySequentialMessages(
                [`Muito prazer, **${userData.nome}**! Qual é a sua formação inicial (graduação)?`],
                session
            );
        ...
        if (onboardingState === 'DEGREE') {
            ...
            await displaySequentialMessages(
                [`Legal! E em que ano você concluiu essa graduação?`],
                session
            );
        ...
        if (onboardingState === 'YEAR') {
            ...
            await displaySequentialMessages(
                [`Você já possui registro ativo no CREA?`],
                session
            );
            if (currentChatSession === session) {
        ...
        if (onboardingState === 'HAS_CREA') {
            ...
            await displaySequentialMessages(
                [`Perfeito. Em qual estado (sigla) você busca a extensão de atribuição?`],
                session
            );
        ...
        if (onboardingState === 'STATE') {
            ...
            await displaySequentialMessages(
                [`Tudo certo! Como posso te ajudar com o seu processo hoje?`],
                session
            );
            if (currentChatSession === session) {
    ...
    async function processLLMMessage(userMessage) {
        const response = await fetch('/api/crea/chat', {
            method: 'POST',
    ...
    async function extractNameViaLLM(userMessage) {
        const response = await fetch('/api/crea/extract-name', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
